...
module.exports = function git(module, env, next) {
  ...
  if (
    ...
    return error(
      `Publish option 'gitCommit' must be a string or a function or a boolean`
    )

  if (!gitCommit) return next(null, `no use git`)

  let gitMessage = isString(gitCommit)
    ? gitCommit
    : isFn(gitCommit)
      ? isString((gitCommitFn = gitCommit(module, env, this)))
        ? gitCommitFn
        : 'update'
      : 'update'
