...
module.exports = function recover(module, env, next) {
    let { _history, recoverTemplate, ssh2shell, remotePath, ssh, shellTrashPath, preCommands, postCommands, _recover: { previous, length = 5 } } = module;
    if (ssh.userName || ssh.user) {
        warning("ssh配置中的'userName'与'user'请统一配置为'username'，详情参考ssh2库(https://github.com/mscdex/ssh2)", 'recover', false, false);
    };
    ...
    if (isEmpty(_history)) return warning('The history record is empty', 'recover', true);
    if (_history.length === 1) return warning('There is only one history record that dont need to recover', 'recover', true);

    remotePath = path.posix.join(remotePath, '.');
    shellTrashPath = shellTrashPath ? path.posix.join(shellTrashPath, '.') : '/tmp/fjpublishTrashDir';

    if (!/^(\/[^\/\s]+){2,}$/.test(remotePath)) return error(`Publish environment '${env}' option 'remotePath' file path is no vaild, the file path must be more than two level directory, and must be absolute path`, 'recover');
    if (shellTrashPath && !/^(\/[^\/\s]+){2,}$/.test(shellTrashPath)) return error(`Publish environment '${env}' option 'shellTrashPath' file path is no vaild, the file path must be more than two level directory, and must be absolute path`, 'recover');

    let p = previous ? Promise.resolve({ index: 1 }) : inquirer.prompt([{
        type: 'list',
        name: 'index',
        message: 'Please select the version you want to recover',
        choices: _history.slice(1, +length + 1).map((v, k) => ({
            name: recoverTemplate ? recoverTemplate(module, v, chalk) : `${chalk.gray(v.tag)}  ${v._backupExt}  ${v.gitMessage || ''}`,
            value: k + 1
        })),
    }]);
