...
function Fjpublish(config, opt) {
  assert(
    ...
    'error',
    `You must upgrade node to ${packageConfig.engines.node} to use fjpublish`
  )
    ...
    let dealHook = function(hooks, direction) {
      ...
      return hooks.forEach(v => {
        ...
        assert(
          isString(when),
          'error',
          "Hook function must hava a key 'when' to tell fjpublish when to use this hook"
        )
 ...
 * @return {Fjpublish}
 */
Fjpublish.prototype.start = function(adjust) {
  ...
  async[parallel ? 'concat' : 'concatSeries'](
    ...
    async.reflect((key, callback) => {
      modules[key]._timeStamp = date('YmdHis')
    ...
    (err, res, body) => {
      if (!err && res.statusCode === 200) {
        ...
        if (semver.lt(localVersion, latestVersion)) {
          console.log()
          console.log(
            chalk.yellow('A newer version of fjpublish is available.')
          )
          console.log()
          ...
          console.log()
          console.log(
            "Visit 'https://github.com/zczhangchao51/fjpublish/releases' for more update information"
          )
          console.log()
        }
      }
