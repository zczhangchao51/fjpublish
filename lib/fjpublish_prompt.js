...
module.exports = function prompt(module, env, next) {
    ...
    function promptParent(callback) {
        let { _prompts = [], prompts: parentPrompt, _promptSyncModule = [], promptSyncModule = [], promptIgnore } = this._metadata;
    ...
    function promptModule(callback) {
        async.eachSeries(Object.keys(modules), (env, done) => {
            ...
            let promptModuleArr = sourcePrompt && modules[env].hasOwnProperty('prompts') ? sourcePrompt : [];
