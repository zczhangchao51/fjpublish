...
module.exports = function builder(module, env, next) {
    ...
    if (isUndefined(buildCommand)) return error(`Publish environment '${env}' option 'buildCommand' is required`);

    if (!isString(buildCommand)) return error(`Publish environment '${env}' option 'buildCommand' must be a string`);

    if (this._metadata.parallel) logger.assert(Object.values(this._metadata.modules).every(v => v.buildCommand === buildCommand), 'error', 'If the task is concurrent, the release command must be consistent', 'builder');
    ...
    let p = this[promiseName] ? this[promiseName] : this[promiseName] = new Promise((resolve, reject) => {
        ...
        let cp = spawn(process.platform === 'win32' ? 'npm.cmd' : 'npm', ['run', buildCommand]);
