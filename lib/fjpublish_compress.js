...
module.exports = function compress(module, env, next) {
  ...
  if (module.diff && module.nohistory)
    warning(
      `File diff dependency on history, please dont set 'nohistory'`,
      false,
      () => {}
    )
  ...
  if (isUndefined(localPath) && isUndefined(localPathEntries))
    return error(
      `Publish environment '${env}' option 'localPath' and 'localPathEntries' exist at least one. if you want to publish all files in the current directory, please set 'localPath' to '.'`
    )

  if (localPath && !isString(localPath))
    return error(
      `Publish environment '${env}' option 'localPath' must be a string`
    )
  if (
    localPathEntries &&
  ...
  if (localTarFileDir && !isString(localTarFileDir))
    return error(
      `Publish environment '${env}' option 'localTarFileDir' must be a string`
    )
  if (tarFilename && !isString(tarFilename))
    return error(
      `Publish environment '${env}' option 'tarFilename' must be a string`
    )
  if (tarFilename && !/^\w+$/.test(tarFilename))
    return error(
      `Publish environment '${env}' option 'tarFilename' name is illegal, e.g. example.tar.gz only white example`
    )

  if (isString(localPathEntries)) localPathEntries = [localPathEntries]
  if (localPathEntries && localPathEntries.some(path.isAbsolute))
    return error(
      `Publish environment '${env}' option 'localPathEntries' must all be relative paths, and all path must in the current working directory`
    )
  if (isString(localPathIgnore)) localPathIgnore = [localPathIgnore]
  let _compressHash = md5(
    []
      .concat(isFn(localPathIgnore) ? Math.random() : localPathIgnore || [])
      .concat(localPath || 'localPath')
      .concat(localPathEntries || 'localPathEntries')
      .concat(localTarFileDir || 'localTarFileDir')
      .concat(tarFilename || 'tarFilename')
      .concat(diff ? Math.random() : 'diff')
      .concat(nobuild ? 'nobuild' : buildCommand || 'buildCommand')
      .join('')
  ).slice(0, 5)

  module.localTarFileDir = localTarFileDir
    ? path.resolve(localTarFileDir)
    : os.tmpdir()
  ...
  let p = this[hashPromiseName]
    ...
    : (this[hashPromiseName] = new Promise((resolve, reject) => {
        ...
        let filename = `${
          tarFilename ? tarFilename : 'fjcompress' + curIndex
        }.tar.gz`
        this._compressIndex++
        ...
        option.ignore = name => {
          ...
          if (isArray(localPathIgnore) && !boolean)
            boolean = !!multimatch([name], localPathIgnore, { dot: true })
              .length
          if (isFile && !boolean && !nohistory) {
            ...
            if (
              diff &&
              _latestFilesHash &&
              _latestFilesHash[name] &&
              fileContentHash === _latestFilesHash[name]
            )
              boolean = true
        ...
        if (localPathMapStream) option.mapStream = localPathMapStream
        option.map = localPathMap
          ? localPathMap
          : header => {
              header.mode = header.type === 'directory' ? 0755 : 0644 // change permission
              return header
            }
        let pack = tar.pack(localPath || '.', option)
        let gzStream = pack.pipe(zlib.createGzip())
        gzStream.pipe(
          fs.createWriteStream(path.join(module.localTarFileDir, filename))
        )
