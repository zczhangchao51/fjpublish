...
module.exports = function sftp(module, env, next) {
    ...
    let p = this[hashPromiseName] ? this[hashPromiseName] : this[hashPromiseName] = new Promise((resolve, reject) => {
        ...
        client.on('transfer', (buffer, uploaded, total) => {
            msg = chalk.gray('Uploading ' + (uploaded / total * 100).toFixed(1) + '%');
            process.stdout.write('\b'.repeat(msg.length) + msg);
        });
        client.on('end', () => {
            process.stdout.write('\b'.repeat(msg.length) + 'Uploading success');
            resolve(null);
        });
        client.on('error', err => {
            process.stdout.write('\b'.repeat(msg.length) + 'Uploading failures');
            resolve(err);
        });
    }).catch(next);
