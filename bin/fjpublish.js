...
program
    ...
    .action((cmd) => {
        ...
        let p = fs.existsSync(confPath) ? inquirer.prompt([{
            type: 'confirm',
            name: 'init',
        ...
        p.then(({ init }) => {
            ...
            let fileStream = fs.createReadStream(path.join(__dirname, '../lib/fjpublish.config.js'));
...
program
    ...
    .option('-k, --key <path>', 'Select the pubic keys that need to be authenticated')
    .action((server, { key }) => {
        COMMAND = 'auth';
        let defaultKey = `${os.homedir()}/.ssh/id_rsa.pub`;
        if (!key) assert(fs.existsSync(defaultKey), 'error', "File '~/.ssh/id_rsa.pub' is not found, please check or select a pubic keys that need to be authenticated");
...
program
    ...
    .action((env, cmd) => {
        ...
        if (env) {
            ...
            selectEnv(modules, 'Please select the environment to publish?', multiple).then((answers) => {
                if (isArray(answers.env) && !answers.env.length) return warning('Not to select');
                envMainFunction(config, isArray(answers.env) ? answers.env : [answers.env], cmd);
            });
        };
    })
...
function MAINFUNC(config, env, cmd) {
    let { use = { pull, prompt, git, builder, compress, sftp, shell, push }, ssh, commit, rebase, tag, push: gitpush, nobuild, nobackup, nohistory, merge, diff, prompt: usePrompt, check, parallel, parent, arg, cmd: command } = cmd;
    let extend = {};
    env = env.map(v => ({
        env: v,
...
function sshParse(str, env) {
    let exec = /^(?:([^:]+):)?([a-zA-Z0-9\-\._]+)@([^:#]+)(?::([0-9]+))?(?:#(.+))?$/.exec(str);
    ...
    if (env.length === 1 && exec && exec[1]) warning("When you publish only one environment, the --ssh parameter 'env' is not required", false);
    if (env.length > 1 && !ssh.env) error("When multiple environments are selected, the --ssh parameter 'env' should clearly indicate which environment is set up");
    if (env.length > 1 && !env.includes(ssh.env)) error('The env that --ssh set up  is not include in this publish task');
    return isUndefined(ssh.password) ? inquirer.prompt([{
        type: 'password',
        name: 'password',
